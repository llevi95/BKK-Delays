...
def refresh_static_if_stale():
    ...
    if (
        last is not None
        and (pendulum.now("UTC") - pendulum.instance(last)).in_hours() < 24
    ):
    ...
    now = datetime.now(timezone.utc).replace(
        tzinfo=None
    )  # naive: avoids MSSQL TIMESTAMP/rowversion mapping
    for name, parse in (
        ("routes", static.parse_routes),
...
with DAG(
    ...
    t_static = PythonOperator(
        task_id="refresh_static_if_stale", python_callable=refresh_static_if_stale
    )
